...
class _AffectationViewState extends State<AffectationView> {
  ...
  void showConfirmationDialog(Function callback) {
    if (tableData.isEmpty) {
      ...
      // Vérifier si chaque vendeur utilise le même véhicule dans toutes les lignes
      String firstVehicle = tableData[0]['vehicule'];
      bool sameVehicle =
          tableData.every((row) => row['vehicule'] == firstVehicle);
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: const Text(
                  'Chaque vendeur doit travailler avec le même véhicule dans toutes les lignes.'),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.of(context).pop();
